...
function displayTable() {
  ...
  ref.once("value").then(function(snapshot) {
    var tr =
      "<thead><th>ID</th><th>SKU</th><th>ASIN</th><th>Inventory</th><th>Price CND</th><th>Weigth lb</th></thead><tbody>";
    $("#myTable").append(tr);
...
function displayRMATable() {
  ...
  ref.once("value").then(function(snapshot) {
    var tr =
      "<thead><th>#</th><th>SKU</th><th>Manufactror No.</th><th>RMA Inventory</th><th>New Received</th><th>Tested</th><th>Action</th></thead><tbody>";
    $("#myTable").append(tr);
    ...
    snapshot.forEach(function(childSnapShot) {
      ...
      if (obj.hasChild("RMAInv")) {
        ...
        var td4 =
          "<td><a id='" +
          rowNum +
          "'href='#' onclick='openModal(this.id)'>Edit</a>&nbsp;&nbsp;<a href='#''>Delete</a></td></tr>";
        rowNum++;
  ...
  $("#functionDiv").html(
    "<button type='button' class='btn btn-success' style='float: right;'' data-toggle='modal' data-target='.bd-example-modal-lg'>Add Memory</button>"
  );
}
...
function openModal(row) {
  ...
  var received = document.getElementById("myTable").rows[row].cells[4]
    .innerHTML;
  var testedNum = document.getElementById("myTable").rows[row].cells[5]
    .innerHTML;
...
function updateData(spreadsheetTable) {
  ...
  for (var i = 0; i < spreadsheetTable.length; i++) {
    ...
    console.log(
      spreadsheetTable[i].SKU +
        spreadsheetTable[i].Price +
        spreadsheetTable[i].LocalInv
    );
    firebase
      .database()
      .ref("RAM/" + memSKU)
